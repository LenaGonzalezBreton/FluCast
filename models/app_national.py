...
def entrainer_et_predire(df_historique):
    ...
    # On ne garde que les départements métropolitains pour la carte
    codes_metro = [str(d).zfill(2) for d in range(1, 96)]
    codes_metro.remove('20') # On enlève la Corse ancienne numérotation
    codes_metro.extend(['2A', '2B'])

    df_metro = df_historique[df_historique['code_departement'].isin(codes_metro)].copy()
    ...
    if predictions_futures:
        ...
        return df_metro[df_metro['annee_semaine'] == df_metro['annee_semaine'].max()].copy()
...
def charger_donnees(chemin_fichier):
    ...
    try:
        ...
        # Gestion de la Corse pour la fusion avec le GeoJSON
