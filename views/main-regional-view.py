...
st.set_page_config(
    page_title="Dashboard Grippe Grand Est",
    page_icon="🌡️",
    layout="wide"
)
...
st.title("🌡️ Thermomètre Grippal Prédictif - Région Grand Est")

# Chargement et traitement des données
df_historique = app.charger_donnees("data/clean-data/donnees_analytiques_grand_est.csv")

if not df_historique.empty:
    df_avec_predictions = app.entrainer_et_predire(df_historique)
    df_final_pour_affichage = app.calculer_score(df_avec_predictions)

    # Création des onglets pour une navigation claire
    tab1, tab2, tab3 = st.tabs(["**Vue d'Ensemble (Carte)**", "**Analyse par Département**", "**À Propos du Projet**"])
    ...
    with tab1:
        ...
        st.info(
            ...
            **Comment lire cette carte ?** Chaque département est coloré selon notre **score de tension prédictif**. 
            ...
            Ce score est calculé en combinant les **cas de grippe prédits par notre modèle** et la **vulnérabilité** de la population (basée sur le taux de vaccination passé).
            """
        )
        ...
        # Indicateurs clés pour toute la région
        ...
        col1.metric("Score de Tension Moyen Régional", f"{score_moyen:.2f}",
                    help="Le score moyen sur 1 pour toute la région. Plus il est proche de 1, plus le risque est élevé.")
        col2.metric("Total Cas Prédits (S+1)", f"{cas_predits_total}",
                    help="La somme des cas (urgences + SOS médecins) attendus pour la semaine prochaine dans toute la région, selon notre modèle.")
        col3.metric("Tendance Moyenne des Cas", f"{tendance_moyenne:+.1%}",
                    help="L'évolution moyenne des cas dans la région par rapport à la semaine précédente.")

        # Carte choroplèthe
        geojson = app.get_geojson()
        if geojson:
            fig = px.choropleth_mapbox(
                ...
                mapbox_style="carto-positron", zoom=6.5, center={"lat": 48.6921, "lon": 6.1844}, opacity=0.7,
    ...
    with tab2:
        ...
        st.info(
            "Ce graphique montre l'évolution historique des cas (urgences + SOS médecins) pour le département sélectionné. Il permet d'identifier les pics saisonniers passés et de mettre en contexte la situation actuelle.")
    ...
    st.warning(
        "Les données n'ont pas pu être chargées. Veuillez vérifier que le fichier 'clean-data/donnees_analytiques_grand_est.csv' existe.")
